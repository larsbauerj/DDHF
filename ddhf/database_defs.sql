...
ALTER TABLE `items` ADD
    CONSTRAINT `donatorid_id_refs_donatorid_67cdc20c`
    FOREIGN KEY (`donatorid_id`) REFERENCES `donators` (`donatorid`);

ALTER TABLE `items` ADD
    CONSTRAINT `fileid_id_refs_fileid_5d9b9cc2`
    FOREIGN KEY (`fileid_id`) REFERENCES `files` (`fileid`);
...
;
ALTER TABLE `items` ADD
    CONSTRAINT `producerid_id_refs_producerid_42b09272`
    FOREIGN KEY (`producerid_id`) REFERENCES `producers` (`producerid`);
...
;
ALTER TABLE `items_itemsubject` ADD
    CONSTRAINT `items_id_refs_itemid_2f3bc1cb`
    FOREIGN KEY (`items_id`) REFERENCES `items` (`itemid`);

ALTER TABLE `items_itemsubject`
    ADD CONSTRAINT `subjects_id_refs_fileid_7734f65d`
    FOREIGN KEY (`subjects_id`) REFERENCES `subjects` (`fileid`);
...
;
ALTER TABLE `items_itempicture` ADD
    CONSTRAINT `items_id_refs_itemid_72227d75`
    FOREIGN KEY (`items_id`) REFERENCES `items` (`itemid`);

ALTER TABLE `items_itempicture` ADD
    CONSTRAINT `pictures_id_refs_pictureid_45e705cb`
    FOREIGN KEY (`pictures_id`) REFERENCES `pictures` (`pictureid`);
...
;
ALTER TABLE `producers_itemlist` ADD
    CONSTRAINT `producers_id_refs_producerid_149c1510`
    FOREIGN KEY (`producers_id`) REFERENCES `producers` (`producerid`);

ALTER TABLE `producers_itemlist` ADD
    CONSTRAINT `items_id_refs_itemid_6c4b62fb`
    FOREIGN KEY (`items_id`) REFERENCES `items` (`itemid`);
COMMIT;
