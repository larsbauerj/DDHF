...
con = Connect(db=settings.DATABASE_NAME,
...
def truncstrg(strg, size=40, more="..."):
  """Trunc <strg> to <size> ending with <more> if trunced""" 
  strg = strg.replace(", ,", "")
  if len(strg) > size:
...
class Donators(models.Model):
  ...
  donatorinstitution = models.CharField('firma/institution', help_text="", max_length=255, null=True, blank=True, db_index=True)
  ...
  def __unicode__(self):
    if self.donatorinstitution.strip():
      return truncstrg(", ".join((
              self.donatorname,
              self.donatorinstitution,
              ))
            )
...
class Pictures(models.Model):
  ...
  def widget(self):
    html = '<a href="%s" target=_blank ><img title="%s" style="border: 4px solid lightblue; display:inline;" src="%s" height="150" /></a>'
    return html  % (self.pictureoriginal.url,escape(self.picturetext) ,self.picturelow.url)

  def picture_tag(self):
    html = '<img title="%s" src="%s" />'
    return html % (escape(self.picturetext), get_tinyname(self))
  ...
  def save(self, *args, **kwargs):
    ...
    sql = """INSERT INTO pictures (
           ...
           pictureregisteredby, lastmodified, picturelow,
           picturemedium, pictureoriginal
         )
         VALUES
         (%s,%s,%s,%s,%s,%s,%s,%s)
        """
    ...
    args = (self.pictureid,
        ...
        self.lastmodified,
        open(filenamelow, "rb").read(),
        open(filenamemedium, "rb").read(),
  ...
  def __unicode__(self):
    return escape(self.picturetext)
    #return truncstrg(", ".join((
    #          "%s" % (self.pictureid),
    #          self.picturetext,
    #          )),
...
class Subjects(models.Model):
  ...
  def __unicode__(self):
    return truncstrg( self.subjecttitle)
...
class Producers(models.Model):
  ...
  def __unicode__(self):
    return truncstrg( self.producertitle)
...
class Status(models.Model):
  class Meta:
    ...
    ordering = ['nummer',]
    unique_together = (("nummer", "status",))
  nummer  = models.IntegerField(help_text=u"værdi", blank=True, null=False, default=0)
  block   = models.CharField(max_length=255, help_text=u"område på siden", blank=True)
  status  = models.CharField(max_length=255, help_text=u"status på siden", blank=True)

  def __unicode__(self):
    return self.block + " " + self.status

def get_status():
  """
  indlæser status tabellen og reskonstruerer den som tuple af 2 tupler
   ((<blok>, ((<nummer>, <status>),...),
      ...))
  således at formatet passer til strukuren for en choice menu
  #status_as = (
  #   ("publiseret", (
  #     (0, 'godkendt'),
  #   ),
  ...
  # unique blocks
  blocks = [s.block for s in status]
  unique_blocks = []
  for b in blocks:
    if b in unique_blocks: continue
    unique_blocks.append(b)
  status_as = []
  for b in unique_blocks:
    status_as.append((b , tuple((s.nummer, s.status) for s in status if s.block == b)))
  status_as = tuple(status_as)
  return status_as


class Items(models.Model):
  status_as = get_status()
  ...
  def view_pictures(self):
    return "".join(p.widget() for p in PictureList.objects.filter(item_id=self.itemid).order_by('orden'))

  view_pictures.label = None
  view_pictures.short_description = "valgte billeder"
  view_pictures.allow_tags = True


  def get_pictures(self):
    html = '<img border="1" class="tiny_img" title="%s" src="%s" />'
    return "".join( html % (escape(p.picturetext), get_tinyname(p)) for p in self.itempicture.select_related().order_by()) 
  ...
  def piclist(self):
    html = '<img border="1" class="tiny_img" title="%s" src="%s" />'
    return "".join( html % (escape(pic.picturetext()), pic.tinyname()) for pic in PictureList.objects.filter(item_id=self.itemid).order_by('orden'))

  piclist.short_description = "Billeder"
  piclist.allow_tags = True
  ...
  fileid             = models.ForeignKey(Files, db_column='fileid', null=False, verbose_name="sag nr.")
  ...
  itemdatingto       = models.DateField(verbose_name="Datering til", null=True, blank=False)
  producerid         = models.ForeignKey(Producers, db_column="producerid", verbose_name='producent', help_text="producent/fabrikant/forfatter", null=True, blank=True)
  itemacquiretype    = models.IntegerField(verbose_name="modtaget som", choices=acquiretype_as, null=True, blank=False, default=1)
  itemdepositeduntil = models.DateField(verbose_name="deponeret/udlånt indtil", null=True, blank=True)
  donatorid          = models.ForeignKey(Donators, db_column="donatorid", verbose_name='donator', null=True, blank=True)
  ...
  itemregistered     = models.DateField(verbose_name="registreringsdato", null=False, blank=True)
  ...
  # XXX Nyt felt
  itemlocation       = models.TextField("pladsering", db_column='itemlocation', blank=True)
  ...
  itemusedwhereid    = models.ForeignKey(Sted, null=True, blank=True, db_column='itemusedwhereid', verbose_name="brugt i geografisk område")
  ...
  itemusedendfrom    = models.DateField(verbose_name="udgået af brug/nedtaget fra", null=True, blank=True)
  itemusedendto      = models.DateField(verbose_name="udgået af brug/nedtaget til", null=True, blank=True)
  ...
  iteminternal       = models.IntegerField("intern", help_text="forenigens aktiv", null=True, blank=True)
  ...
  # itempicture_list   = models.ManyToManyField(Pictures, through="PictureList", verbose_name="billed liste" , null=True, blank=True)
  # itempicture        = models.ManyToManyField('PictureList', verbose_name="billed liste" , null=True, blank=True)
...
class PictureList(models.Model):
  ...
  itemid.short_description = "Itemid"
  itemid.allow_tags        = True

  def picturetext(self):
    return escape(self.picture.picturetext)

  picturetext.short_description = "Billedetekst"

  def picturelow_url(self):
    return self.picture.picturelow.url
  picturelow_url.short_description = "Billede"

  def widget(self):
    html = '<a href="%s" target=_blank ><img title="%s" style="border: 4px solid lightblue; display:inline;" src="%s" height="150" /></a>'
    return html % (self.picture.pictureoriginal.url, escape(self.picture.picturetext) ,self.picture.picturelow.url)
  widget.short_description = "Billede"


  def tinyname(self):
    return get_tinyname(self.picture)
  tinyname.short_description = "Billede"

  def picture_tag(self):
    return self.picture.picture_tag()
  ...
  def __unicode__(self):
    return truncstrg(", ".join((
              "%s" % (self.picture.pictureid),
              escape(self.picture.picturetext),
              )),
            )
